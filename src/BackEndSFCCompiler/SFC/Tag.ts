...
export function Tag (html :string, position :number) {
	...
	if ( html.startsWith('<', position) ) {
		...
		if ( IS_TAG.test(rest) ) {
			...
			if ( endSolidus ) {
				if ( data ) { throw SyntaxError(`结束标签中存在属性`); }
			...
			if ( data ) {
				...
				if ( data.endsWith('/') ) { throw SyntaxError(`标签结尾处的“/”有歧义，无法达到标注标签为自闭合的目的，而是会作为最后一个无引号属性值的结束字符`); }
			}
	...
	if ( rest ) {
		...
		const raw :string = rest.slice(position, end);
		Entities.unescape(raw);
