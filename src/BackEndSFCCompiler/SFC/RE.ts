...
export const NameAs__Key__ = (Name :string) :string => {
	...
	throw SyntaxError(`“${Name}”不满足自动生成动态值的条件`);
};
...
export const startsWithUpperCase = test.bind(/^[A-Z]/);
...
export const ATTRIBUTE_NAME_VALUE = newRegExp<1 | 2>`
	(${ATTRIBUTE_NAME})
	${ASCII_WHITESPACE}*
	=
	${ASCII_WHITESPACE}*
...
export const TAG = newRegExp<1 | 2 | 3 | 4>`
	^
	<
	(/?)
	(${TAG_NAME})
