...
const getOwnPropertyDescriptor$1 = (
	...
	function getOwnPropertyDescriptor$1 (object, key) {
		var descriptor = /*#__PURE__*/ getOwnPropertyDescriptor(object, key);
...
function Snippet(whole, errorPosition) {
    ...
    if (errorLineEnd < 0) {
        maxLengthOfLineNumber = (errorLineNumber + '').length;
    }
    else {
...
function unescape_or_return(ambiguous_ampersand, inner) {
    if (inner[0] === '#') {
        ...
        if (codePoint === 0x00 || 0xD800 <= codePoint && codePoint <= 0xDFFF || 0x10FFFF < codePoint) {
            return '\uFFFD';
        }
        return fromCodePoint(codePoint in CONTROL_TO_CHAR ? CONTROL_TO_CHAR[codePoint] : codePoint);
    }
    else {
        if (ambiguous_ampersand.endsWith(';') && inner in SEMICOLON_ENTITIES) {
            return SEMICOLON_ENTITIES[inner];
        }
...
function unescape_or_throw(ambiguous_ampersand, named) {
    if (ambiguous_ampersand.length === 1) {
        throw SyntaxError(`孤立的“&”没有作为 HTML 实体存在`);
    }
    if (!ambiguous_ampersand.endsWith(';')) {
        throw SyntaxError(`HTML 实体“${ambiguous_ampersand}”后缺少“;”`);
    }
    if (named) {
        if (named in SEMICOLON_ENTITIES) {
            return SEMICOLON_ENTITIES[named];
        }
    ...
    if (codePoint === 0x00) {
        throw RangeError(`HTML 实体“${ambiguous_ampersand}”数值为空（U+0000）码点`);
    }
    if (codePoint > 0x10FFFF) {
        throw RangeError(`HTML 实体“${ambiguous_ampersand}”数值超出了 Unicode 的最大范围（U+10FFFF）`);
    }
    const unicode = fromCodePoint(codePoint);
    if (NON_SCALAR.test(unicode)) {
        throw RangeError(`HTML 实体“${ambiguous_ampersand}”数值是单个代理对码点（U+D800〜U+DFFF）`);
    }
    if (NONCHARACTER.test(unicode)) {
        throw RangeError(`HTML 实体“${ambiguous_ampersand}”数值是永久未定义字符码点（U+FDD0〜U+FDEF、U+[00-10]FFFE、U+[00-10]FFFF）`);
    }
    if (CONTROL_CHARACTER.test(unicode) || codePoint === 0x0D) {
        throw RangeError(`HTML 实体“${ambiguous_ampersand}”数值是除水平制表（U+0009）换行（U+000A）换页（U+000C）以外的控制字符（U+0000〜U+001F、U+007F〜U+009F）`);
    }
    return unicode;
}
...
const { orderify } = {
    orderify(object) {
        ...
        proxy = newProxy(object, new Keeper(ownKeys$1(object)));
        ...
        if (IS_TAG.test(rest)) {
            ...
            if (endSolidus) {
                ...
                if (selfClosingSolidus) {
                    throw SyntaxError(`结束标签中存在自关闭斜杠`);
                }
            ...
            if (attributesLiteral) {
                for (let name of attributesLiteral.match(ATTRIBUTE)) {
                    ...
                    if (name in attributes) {
                        throw SyntaxError(`标签中出现了重复的属性“${name}”`);
                    }
                    else {
...
class Block extends NULL {
    constructor(blockName, attributes, emitProperties, inner, END_TAG) {
        ...
        if (inner === undefined$1) {
            if (emitProperties) {
                if (attributes.src === undefined$1) {
                    throw SyntaxError(`自闭合功能块元素必须存在 src 属性值`);
                }
                this.src = attributes.src;
                if ('lang' in attributes) {
                    throw SyntaxError(`自闭合功能块元素不支持 lang 属性`);
                }
            }
        }
        else {
            ...
            if (emitProperties) {
                if ('src' in attributes) {
                    throw SyntaxError(`开放功能块元素不能存在 src 属性`);
                }
                if ('lang' in attributes) {
                    if (!attributes.lang) {
                        throw SyntaxError(`开放功能块元素的 lang 属性如果设置，值不能为空`);
                    }
                    this.lang = attributes.lang;
                }
            }
            if (END_TAG && END_TAG.test(inner)) {
                throw SyntaxError(`“${blockName}”块内包含疑似结束标签的内容（注意 .vue 文件需要确保单行/多行解析模式与传统 HTML 流式解析的结果一致）`);
            }
        }
    }
}
...
class Script extends Block {
    ...
    get innerJS() {
        ...
        if (typeof inner !== 'string') {
            throw Error(`自闭合的 script 功能块元素必须自行（根据 src 属性）加载 inner 值`);
        }
        if (this.lang && !JS.test(this.lang)) {
            throw Error(`script 功能块元素如果设置了非 js 的 lang 属性值，那么必须自行提供转译后的 inner，并将 lang 设置为 js`);
        }
        return inner;
    }
}
    ...
    constructor(attributes, inner) {
        ...
        if ('abbr.' in attributes) {
            ...
            if (literal === undefined$1) {
                ...
                if (!SELECTOR.test(literal)) {
                    throw SyntaxError(`style 块的“abbr.”属性语法错误：\n${literal}`);
                }
        ...
        if ('media' in attributes) {
            if (attributes.media === undefined$1) {
                throw SyntaxError(`style 功能块元素的 media 属性必须具有值`);
            }
...
function parseAppend(parentNode_xName, parentNode, V_PRE, FOREIGN) {
    for (;;) {
        ...
        if (xName === 'textarea' || xName === 'title' || xName === 'STYLE') {
            ...
            throw SyntaxError(`Vue 不会将 style 或 script 的任何大小写变种中的内容理解为正常标签嵌套，而是会剔除其内容中的标签、解除 HTML 实体转义后，作为文本内容理解，` +
                `jVue 虽可对其进行转写（比如“<component is="${xName}">”或“<${xName} v-text="..." />”），` +
                `但由于缺乏约定（就连 Vue 本身的这种行为，也是一种边缘情况，既谈不上合理，也不能保证一直如此对待，甚至没有在文档中言明），` +
...
class Content extends Node {
    constructor(inner, abbr) {
        ...
        if (inner) {
            ...
            if (this.firstChild instanceof Text && TNS.test(this.firstChild.data) && SOF_TNS_LT.test(inner)) {
                this.childNodes.shift();
            }
            if (this.lastChild instanceof Text && TNS.test(this.lastChild.data) && GT_TNS_EOF.test(inner)) {
                this.childNodes.pop();
            }
        }
    }
...
class Template extends Block {
    constructor(attributes, inner) {
        if (inner !== undefined$1 && attributes.lang && !HTML.test(attributes.lang)) {
            if (TAG_LIKE.test(inner)) {
                throw SyntaxError(`.vue 文件的 template 块（非 html 状态下）中，存在可能使得正常结束判定结果模糊的标签语法标记`);
            }
        ...
        if ('abbr.' in attributes) {
            const literal = attributes['abbr.'];
            if (literal === undefined$1) {
                throw SyntaxError(`template 功能块元素的“abbr.”属性的缺省值写法还没有实现`);
            }
            else {
                if (!PARTIAL.test(literal)) {
                    throw SyntaxError(`template 块的“abbr.”属性语法错误：\n${literal}`);
                }
                const abbr = create(null);
                for (const pair of literal.split(';')) {
                    const tokens = pair.match(TOKENS);
        ...
        if ('scoped:keys' in attributes) {
            if (attributes['scoped:keys'] === undefined$1) {
                throw SyntaxError(`template 功能块元素的 scoped:keys 属性必须具有值`);
            }
    ...
    get content() {
        ...
        if (typeof this.inner !== 'string') {
            throw Error(`自闭合的 template 功能块元素必须自行（根据 src 属性）加载 inner 值`);
        }
...
class CustomBlock extends Block {
    constructor(blockName, attributes, inner) {
        if (inner === undefined$1) {
            ...
            if (ESCAPABLE_RAW_TEXT_ELEMENTS.test(blockName)) {
                throw SyntaxError(`.vue 文件中的自定义块尚没有明确的语义约定，请避免使用 textarea / title 标签及其大小写变种`);
            }
            if (TAG_LIKE.test(inner)) {
                throw SyntaxError(`.vue 文件的 ${blockName} 自定义块中，存在可能使得正常结束判定结果模糊的标签语法标记`);
            }
...
function parseComponent(sfc, vue) {
    ...
    for (const length = vue.length; index !== length;) {
        ...
        if (blockName === 'template') {
            sfc.template = new Template(tag.attributes, inner);
        }
        else if (blockName === 'style') {
            sfc.styles.push(new Style(tag.attributes, inner));
        }
        else if (blockName === 'script') {
            sfc.script = new Script(tag.attributes, inner);
        }
        else {
            sfc.customBlocks.push(new CustomBlock(blockName, tag.attributes, inner));
        }
        if (index !== length) {
            if (vue.startsWith('\n', index)) {
                ++index;
            }
            else if (!vue.startsWith('<!', index)) {
                throwSyntaxError(`顶级标签的结束标签后的同一行内不应有除注释以外的内容`);
            }
        }
    }
}
...
function NecessaryStringLiteral(body) {
    ...
    if (!without.startsWith(PRE) || !without.endsWith(SUR)) {
    ...
    if (minified.warnings) {
        throw Error(`Terser 压缩警告：\n${minified.warnings.join('\n')}`);
    }
    const vm_c_exp = VM_C_EXP.exec(minified.code);
    if (!vm_c_exp) {
        throw Error(`jVue 内部设计时错误地估计了 Terser 压缩生成的内容格式：\n` + minified.code);
    }
...
class SFC extends NULL {
    constructor(vue) {
        ...
        if (typeof vue === 'string') {
            if (NON_SCALAR.test(vue)) {
                throw Error(`.vue 文件所基于的 HTML 字符流中禁止出现落单的代理对码点（U+D800〜U+DFFF）`);
            }
            this.bom = '';
        }
        else if (isBuffer(vue)) {
            try {
                ({ BOM: this.bom, string: vue } = buffer2object(vue, OPTIONS));
            }
            catch (error) {
                throw Error(`无法解码 Buffer，请确认它是 UTF-8 或 UTF-16 编码的，并且不存在非 Unicode 标量值（U+D800〜U+DFFF 的代理对码点，或超出了 U+10FFFF）`);
            }
        }
        else {
            throw TypeError(`new SFC(vue) 时参数只能是 string 或 Buffer`);
        }
        if (NONCHARACTER.test(vue)) {
            throw Error(`.vue 文件所基于的 HTML 字符流中禁止出现永久未定义字符码点（U+FDD0〜U+FDEF、U+[00-10]FFFE、U+[00-10]FFFF）`);
        }
        if (CONTROL_CHARACTER.test(vue)) {
            throw Error(`.vue 文件所基于的 HTML 字符流中禁止出现除空（U+00）、水平制表（U+09）、换行（U+0A）、换页（U+0C）、回车（U+0D）之外的控制字符（U+00〜U+1F、U+7F〜U+9F）`);
        }
        try {
            this.eol = VUE_EOL(vue);
        }
        catch (error) {
            throw SyntaxError(`.vue 文件的换行符必须是 LF（U+0A）、CRLF（U+0D U+0A）或 CR（U+0D）中的唯一一个` +
                `，而且` + // script 标签内容前
                `不得包含对 JS 是换行符、而对 HTML 和 CSS 不是换行符的 U+2028 或 U+2029，也不得包含对 CSS 是换行符、而对 HTML 和 JS 不是换行符的 U+0C` //，否则源图映射的行号和列号可能出错
            );
        }
        if (CR_LF.test(this.eol)) {
            vue = vue.replace(CR_LF, '\n');
        }
    ...
    export(mode, from = mode === 'default' ? 'j-vue?*' : 'j-vue') {
        ...
        if (mode === 'default') {
            ...
            let code = bom;
            for (const chunk of From(tab, mode, styles, template, from, eol)) {
                code += chunk;
            }
            return code;
        }
