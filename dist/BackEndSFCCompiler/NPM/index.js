	...
	for ( ; ; ) {
		...
		if ( notComponent ) {
			...
			if ( afterColon.includes('-') ) {
				...
				if ( compatible_template && xName===SVG ) { compatible_template = false; }
			}
			else {
		...
		if ( v_pre ) {
			...
			if ( v_if ) {
				if ( 'v-if' in attributes ) {
					if ( 'v-for' in attributes ) { throw Error$1(`v-if 和 v-for 等的优先级在 Vue 2 和 3 中不同，请避免同时使用`); }///if ( compatible_template ) { compatible_template = false; }
			...
			if (
				...
				if ( xName==='style' ) {
					if ( v_pre ) {
						...
						if ( expression ) {
							if ( sheetRef ) {
								if ( sheet.size===sheet.set(sheetRef.ref, expression).size ) { throw Error$1(`出现了重复的同步样式表名“#${sheetRef.ref}#”`); }
								if ( compatible_render || compatible_template ) {
									compatible_render = false;
									compatible_template = false;
								}
...
const Sheets = (sheet                     ) => {
	...
	for ( const { 0: ref, 1: exp } of sheet ) {
		literal += `${ref}:({${findGlobals(parse(exp, parserOptions$1)).names().join(',')}})=>${exp},`;
	}
